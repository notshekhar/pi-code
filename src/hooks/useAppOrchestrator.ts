...
const EXCLUSIVE_HEADER_TOOL_IDS = new Set<ToolId>(["git", "files", "mcp"]);

export function useAppOrchestrator() {
  const sidebar = useSidebar();
  const projectManager = useProjectManager();
  const spaceManager = useSpaceManager();
  const LAST_SESSION_KEY = "pi-code-last-session-per-space";
  ...
  const acpPermissionBehavior = (localStorage.getItem("pi-code-acp-permission-behavior") ?? "ask") as AcpPermissionBehavior;
  ...
  const handleToggleTool = useCallback(
    (toolId: ToolId) => {
      ...
      const isExclusiveHeaderTool = EXCLUSIVE_HEADER_TOOL_IDS.has(toolId);
      settings.setActiveTools((prev) => {
        ...
        if (next.has(toolId)) {
          ...
          if (isExclusiveHeaderTool) {
            for (const otherId of EXCLUSIVE_HEADER_TOOL_IDS) {
              if (otherId !== toolId) next.delete(otherId);
            }
          }
          next.add(toolId);
  ...
  const isBottomDockTool = (id: ToolId) => id === "terminal" || settings.bottomTools.has(id);
  // Side column only includes active COLUMN tools that are NOT placed in the bottom row
  const hasToolsColumn = [...settings.activeTools].some((id) => COLUMN_TOOL_IDS.has(id) && !isBottomDockTool(id)) && !!manager.activeSessionId;
  // Bottom tools row: active COLUMN tools that ARE placed in the bottom row
  const hasBottomTools = [...settings.activeTools].some((id) => COLUMN_TOOL_IDS.has(id) && isBottomDockTool(id)) && !!manager.activeSessionId;
  ...
  }, [sidebar.isOpen, minChatWidth, margins, winFrameBuffer]);
