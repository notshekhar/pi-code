...
export function useNotifications({
  ...
  useEffect(() => {
    ...
    window.addEventListener("pi-code:background-session-complete", onBackgroundComplete as EventListener);
    window.addEventListener("pi-code:background-permission-request", onBackgroundPermission as EventListener);
    return () => {
      window.removeEventListener("pi-code:background-session-complete", onBackgroundComplete as EventListener);
      window.removeEventListener("pi-code:background-permission-request", onBackgroundPermission as EventListener);
    };
  }, [settings]);
}
