...
function readToolsSplitRatios(pid: string): number[] {
  const newKey = `pi-code-${pid}-tools-split-ratios`;
  const existing = readJson<number[]>(newKey, []);
  if (existing.length > 0) return existing;

  // Migrate from old single-ratio key
  const oldKey = `pi-code-${pid}-tools-split`;
...
function readToolOrder(pid: string): ToolId[] {
  const stored = readJson<ToolId[]>(`pi-code-${pid}-tool-order`, []).filter((id) => VALID_TOOL_IDS.has(id));
...
function engineModelKey(pid: string, engine: EngineId): string {
  return `pi-code-${pid}-model-${engine}`;
}

function legacyModelKey(pid: string): string {
  return `pi-code-${pid}-model`;
}
...
function readProjectLayoutState(pid: string): ProjectLayoutState {
  ...
  const loaded: ProjectLayoutState = {
    rightPanelWidth: readNumber(`pi-code-${pid}-right-panel-width`, DEFAULT_RIGHT_PANEL, MIN_RIGHT_PANEL, MAX_RIGHT_PANEL),
    toolsPanelWidth: readNumber(`pi-code-${pid}-tools-panel-width`, DEFAULT_TOOLS_PANEL, MIN_TOOLS_PANEL, MAX_TOOLS_PANEL),
    toolsSplitRatios: readToolsSplitRatios(pid),
    rightSplitRatio: readNumber(`pi-code-${pid}-right-split`, DEFAULT_SPLIT, MIN_SPLIT, MAX_SPLIT),
    bottomToolsHeight: readNumber(`pi-code-${pid}-bottom-tools-height`, DEFAULT_BOTTOM_HEIGHT, MIN_BOTTOM_HEIGHT, MAX_BOTTOM_HEIGHT),
    bottomToolsSplitRatios: readJson<number[]>(`pi-code-${pid}-bottom-tools-split-ratios`, []),
  };
...
export function useSettings(projectId: string | null, engine: EngineId = "claude"): Settings {
  ...
  const setPlanMode = useCallback((enabled: boolean) => {
    ...
    localStorage.setItem("pi-code-plan-mode", String(enabled));
  }, []);

  const [permissionMode, setPermissionModeRaw] = useState(() =>
    (() => {
      const stored = localStorage.getItem("pi-code-permission-mode");
  ...
  const setPermissionMode = useCallback((mode: string) => {
    ...
    if (mode === "plan") {
      setPlanModeRaw(true);
      localStorage.setItem("pi-code-plan-mode", "true");
      mode = DEFAULT_PERMISSION_MODE;
    }
    setPermissionModeRaw(mode);
    localStorage.setItem("pi-code-permission-mode", mode);
  }, []);

  const [acpPermissionBehavior, setAcpPermissionBehaviorRaw] = useState<AcpPermissionBehavior>(() => {
    const stored = localStorage.getItem("pi-code-acp-permission-behavior");
  ...
  const setAcpPermissionBehavior = useCallback((behavior: AcpPermissionBehavior) => {
    ...
    localStorage.setItem("pi-code-acp-permission-behavior", behavior);
  }, []);

  const [thinking, setThinkingRaw] = useState(() =>
    readBool("pi-code-thinking", true),
  );
  const setThinking = useCallback((on: boolean) => {
    setThinkingRaw(on);
    localStorage.setItem("pi-code-thinking", String(on));
  }, []);

  const [claudeEffort, setClaudeEffortRaw] = useState<ClaudeEffort>(() => {
    const stored = localStorage.getItem("pi-code-claude-effort");
  ...
  const [gitCwd, setGitCwdRaw] = useState<string | null>(() =>
    localStorage.getItem(`pi-code-${pid}-git-cwd`),
  );
  const setGitCwd = useCallback(
    (nextPath: string | null) => {
      setGitCwdRaw(nextPath);
      const key = `pi-code-${pid}-git-cwd`;
  ...
  const [activeTools, setActiveToolsRaw] = useState<Set<ToolId>>(() => {
    const arr = readJson<ToolId[]>(`pi-code-${pid}-active-tools`, []).filter((id) => VALID_TOOL_IDS.has(id));
    return new Set(arr);
  });
  const setActiveTools = useCallback(
    (updater: Set<ToolId> | ((prev: Set<ToolId>) => Set<ToolId>)) => {
      setActiveToolsRaw((prev) => {
        ...
        localStorage.setItem(`pi-code-${pid}-active-tools`, JSON.stringify([...next]));
        return next;
      });
    },
    [pid],
  ...
  const saveRightPanelWidth = useCallback(() => {
    ...
    localStorage.setItem(`pi-code-${pid}-right-panel-width`, String(rightPanelWidthRef.current));
  }, [pid]);
  ...
  const saveToolsPanelWidth = useCallback(() => {
    ...
    localStorage.setItem(`pi-code-${pid}-tools-panel-width`, String(toolsPanelWidthRef.current));
  }, [pid]);
  ...
  const saveToolsSplitRatios = useCallback(() => {
    ...
    localStorage.setItem(`pi-code-${pid}-tools-split-ratios`, JSON.stringify(toolsSplitRatiosRef.current));
  }, [pid]);
  ...
  const setToolOrder = useCallback(
    (updater: ToolId[] | ((prev: ToolId[]) => ToolId[])) => {
      setToolOrderRaw((prev) => {
        const next = typeof updater === "function" ? updater(prev) : updater;
        localStorage.setItem(`pi-code-${pid}-tool-order`, JSON.stringify(next));
        return next;
      });
    },
    [pid],
  ...
  const saveRightSplitRatio = useCallback(() => {
    ...
    localStorage.setItem(`pi-code-${pid}-right-split`, String(rightSplitRatioRef.current));
  }, [pid]);

  // ── Collapsed repos ──

  const [collapsedRepos, setCollapsedRepos] = useState<Set<string>>(() => {
    const arr = readJson<string[]>(`pi-code-${pid}-collapsed-repos`, []);
    return new Set(arr);
  });
  const toggleRepoCollapsed = useCallback(
    (path: string) => {
      setCollapsedRepos((prev) => {
        ...
        localStorage.setItem(`pi-code-${pid}-collapsed-repos`, JSON.stringify([...next]));
        return next;
      });
    },
    [pid],
  ...
  const [suppressedPanels, setSuppressedPanels] = useState<Set<ToolId>>(() => {
    const arr = readJson<ToolId[]>(`pi-code-${pid}-suppressed-panels`, []);
    return new Set(arr);
  });
  const suppressPanel = useCallback(
    (id: ToolId) => {
      setSuppressedPanels((prev) => {
        const next = new Set(prev);
        next.add(id);
        localStorage.setItem(`pi-code-${pid}-suppressed-panels`, JSON.stringify([...next]));
        return next;
      });
    },
    [pid],
  ...
  const [bottomTools, setBottomToolsRaw] = useState<Set<ToolId>>(() => {
    const arr = readJson<ToolId[]>(`pi-code-${pid}-bottom-tools`, []).filter((id) => VALID_TOOL_IDS.has(id));
    return new Set(arr);
  });
  const moveToolToBottom = useCallback(
    (id: ToolId) => {
      setBottomToolsRaw((prev) => {
        const next = new Set(prev);
        next.add(id);
        localStorage.setItem(`pi-code-${pid}-bottom-tools`, JSON.stringify([...next]));
        return next;
      });
    },
    [pid],
  ...
  const saveBottomToolsHeight = useCallback(() => {
    ...
    localStorage.setItem(`pi-code-${pid}-bottom-tools-height`, String(bottomToolsHeightRef.current));
  }, [pid]);
  ...
  const saveBottomToolsSplitRatios = useCallback(() => {
    ...
    localStorage.setItem(`pi-code-${pid}-bottom-tools-split-ratios`, JSON.stringify(bottomToolsSplitRatiosRef.current));
  }, [pid]);
  ...
  useEffect(() => {
    ...
    setGitCwdRaw(localStorage.getItem(`pi-code-${pid}-git-cwd`));

    const tools = readJson<ToolId[]>(`pi-code-${pid}-active-tools`, []);
    setActiveToolsRaw(new Set(tools));
    setToolOrderRaw(readToolOrder(pid));

    const repos = readJson<string[]>(`pi-code-${pid}-collapsed-repos`, []);
    setCollapsedRepos(new Set(repos));

    const suppressed = readJson<ToolId[]>(`pi-code-${pid}-suppressed-panels`, []);
    setSuppressedPanels(new Set(suppressed));

    const bottom = readJson<ToolId[]>(`pi-code-${pid}-bottom-tools`, []).filter((id) => VALID_TOOL_IDS.has(id as ToolId));
