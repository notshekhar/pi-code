...
import type { GitRepoInfo, GitStatus, GitBranch, GitLogEntry, GitFileGroup } from "@/types";
...
export function useGitStatus({ projectPath }: UseGitStatusOptions) {
  ...
  const getDiff = useCallback(
    async (repoPath: string, file: string, staged: boolean, group?: GitFileGroup) => {
      return window.claude.git.diffFile(repoPath, file, staged, group);
    },
    [],
  );
