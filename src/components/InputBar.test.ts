...
describe("InputBar slash command helpers", () => {
  ...
  it("matches dropdown search against labels, descriptions, and ids", () => {
    expect(matchesDropdownQuery("sonnet", "Claude Sonnet 4")).toBe(true);
    expect(matchesDropdownQuery("thinking", "Claude 4", "Extended thinking")).toBe(true);
    expect(matchesDropdownQuery("gpt5mini", "GPT-5 Mini", undefined, "openai/gpt-5-mini")).toBe(true);
    expect(matchesDropdownQuery("opus", "Claude Sonnet 4", "Fast model")).toBe(false);
  });
});
