...
import { ToolPicker, type ToolId } from "./ToolPicker";
...
const JIRA_BOARD_BY_SPACE_KEY = "pi-code-jira-board-by-space";
const HEADER_TOOL_IDS = new Set<ToolId>(["terminal", "git", "files", "mcp"]);
const PINNED_BOTTOM_TOOL_IDS = new Set<ToolId>(["terminal"]);
...
export function AppLayout() {
  ...
  const {
    ...
    activeTodos, bgAgents, hasTodos, hasAgents,
...
Link: ${issue.url}`;
  ...
  const toolbarGit = useGitStatus({ projectPath: activeProjectPath });
  const toolbarDiffStat = toolbarGit.repoStates.reduce(
    (acc, state) => ({
      additions: acc.additions + state.diffStat.additions,
      deletions: acc.deletions + state.diffStat.deletions,
    }),
    { additions: 0, deletions: 0 },
  );
  const formatChangeCount = (value: number) => value.toLocaleString("en-US");
  const showCodexAuthDialog =
    !!manager.activeSessionId &&
    manager.activeSession?.engine === "codex" &&
    manager.codexAuthRequired;

  const embeddedBottomDock = manager.activeSessionId && (() => {
    ...
    const allBottomToolIds = settings.toolOrder.filter((id) =>
      id in bottomToolComponents && (settings.bottomTools.has(id) || PINNED_BOTTOM_TOOL_IDS.has(id)),
    );
    ...
    return (
      <>
        <div
          className={`resize-row group flex h-2 shrink-0 cursor-row-resize items-center justify-center border-t border-border/35 ${!hasBottomTools ? "hidden" : ""}`}
          style={isIsland ? { height: "var(--island-panel-gap)" } : undefined}
          onMouseDown={handleBottomResizeStart}
        >
          <div
            className={`h-px w-10 rounded-full transition-colors duration-150 ${
              isResizing
                ? "bg-foreground/40"
            ...
            return (
              <div key={id} className={isActive ? "contents" : "hidden"}>
                <div
                  ...
                  style={isActive ? { flex: `${bottomRatios[activeIdx]} 1 0%` } : undefined}
                >
                  {bottomToolComponents[id]}
                </div>
                {isActive && activeIdx < bottomCount - 1 && (
                  <div
                    className="resize-col group flex w-2 shrink-0 cursor-col-resize items-center justify-center border-s border-border/35"
                    style={isIsland ? { width: "var(--island-panel-gap)" } : undefined}
                    onMouseDown={(e) => handleBottomSplitStart(e, activeIdx)}
                  >
                    <div
                      className={`h-10 w-px rounded-full transition-colors duration-150 ${
                        isResizing
                          ? "bg-foreground/40"
                          : "bg-transparent group-hover:bg-foreground/25"
                      }`}
  ...
  return (
    ...
    >
      ...
      <div ref={contentRef} className={`flex min-w-0 flex-1 flex-col ${settings.islandLayout ? "m-[var(--island-gap)]" : sidebar.isOpen ? "flat-divider-s" : ""} ${isResizing ? "select-none" : ""}`}>
        ...
        {/* ── Top row: Chat | Right Panel | Tools Column ── */}
        <div className="flex min-h-0 flex-1">
                  ...
                  <ChatHeader
                    ...
                    toolbar={(
                      <ToolPicker
                        ...
                        panelToolFilter={HEADER_TOOL_IDS}
                        pinnedBottomTools={PINNED_BOTTOM_TOOL_IDS}
                          ...
                          files: (
                            <>
                              <span className="font-medium tabular-nums text-emerald-500">
                                +{formatChangeCount(toolbarDiffStat.additions)}
                              </span>
                              <span className="font-medium tabular-nums text-red-500">
                                -{formatChangeCount(toolbarDiffStat.deletions)}
                              </span>
                            </>
                          ),
                        }}
          ...
          {manager.activeSessionId && (() => {
            ...
            const sideToolIds = settings.toolOrder.filter((id) =>
              id in toolComponents && !settings.bottomTools.has(id) && !PINNED_BOTTOM_TOOL_IDS.has(id),
            );
