...
function ModelDropdown({
  ...
  const selectedEffort = activeEffort && (effortOptionsByModel?.[selectedModelId]?.includes(activeEffort) ?? false)
    ? activeEffort
    : undefined;
  const showSearch = modelList.length > SEARCHABLE_DROPDOWN_THRESHOLD;
  const filteredModels = showSearch
    ? modelList.filter((model) => matchesDropdownQuery(query, model.label, model.description, model.id))
    : modelList;
    ...
    >
      ...
      <DropdownMenuContent align="start">
        ...
        {filteredModels.length === 0 && (
          <div className="px-2 py-3 text-xs text-muted-foreground">No models found.</div>
        )}
...
function ConfigDropdown({
  ...
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");
  const showSearch = options.length > SEARCHABLE_DROPDOWN_THRESHOLD;
  const filteredOptions = showSearch
    ? options.filter((option) => matchesDropdownQuery(query, option.name, option.description, option.value))
    : options;
    ...
    >
      ...
      <DropdownMenuContent align="start">
        ...
        {filteredOptions.length === 0 && (
          <div className="px-2 py-3 text-xs text-muted-foreground">No options found.</div>
        )}
        {filteredOptions.map((option) => (
          <DropdownMenuItem
            key={option.value}
            onClick={() => onChange(option.value)}
            className={option.value === currentValue ? "bg-accent" : ""}
          >
            <div>
              <div>{option.name}</div>
              {option.description && (
        ...
        {showACPConfigOptions && acpConfigOptions && acpConfigOptions.length > 0 && onACPConfigChange &&
          acpConfigOptions.map((opt) => {
            ...
            return (
              <ConfigDropdown
                key={opt.id}
                triggerLabel={current?.name ?? opt.currentValue}
                currentValue={opt.currentValue}
                options={flat}
                isProcessing={isProcessing}
                onChange={(value) => onACPConfigChange(opt.id, value)}
              />
            );
          })
        }
...
const SEARCHABLE_DROPDOWN_THRESHOLD = 10;

function normalizeDropdownQuery(value: string): string {
  return value.trim().toLowerCase();
}

export function matchesDropdownQuery(query: string, ...fields: Array<string | null | undefined>): boolean {
  const normalizedQuery = normalizeDropdownQuery(query);
  if (!normalizedQuery) return true;

  return fields.some((field) => {
    if (!field) return false;
    return fuzzyMatch(normalizedQuery, field).match;
  });
}
