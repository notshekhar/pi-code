...
export function TaskTool({
  ...
  message: UIMessage;
  /** When true (nested in ToolGroupBlock), parent already constrains width — skip inner 85% cap. */
  compact?: boolean;
}) {
  ...
  // Keep border/padding/chevron after completion — only the pre-result task line stays minimal.
  const showCardChrome = isRunning || expanded || isCompleted;
  // Same full column width whenever the card chrome is shown (running, expanded, or done).
  const useFullWidth = compact || showCardChrome;
  const rootWidthClass = useFullWidth ? "w-full min-w-0" : "w-full max-w-[85%] min-w-0";

  return (
    <div className={rootWidthClass}>
      <Collapsible open={expanded} onOpenChange={setExpanded}>
        <div className={showCardChrome
          ...
          <CollapsibleTrigger className={`group relative flex w-full items-center gap-2.5 text-[13px] hover:text-foreground transition-colors cursor-pointer overflow-hidden ${
            showCardChrome ? "px-3.5 py-2" : "py-1"
          }`}>
