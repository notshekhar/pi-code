 ...
 * `__pi_code_grab__` marker so the renderer can filter it from normal console output.
 */

// Marker used to identify our messages in the console-message stream.
export const GRAB_MARKER = "__pi_code_grab__";
 ...
 * - Exposes `window.__pi_code_inspector_cleanup__()` for teardown
 */
export function getInspectorScript(): string {
  ...
  if (window.__pi_code_inspector_active__) return;
  window.__pi_code_inspector_active__ = true;

  /* ── Overlay ── */
  var overlay = document.createElement('div');
  overlay.id = '__pi_code_inspector_overlay__';
  ...
  function onClick(e) {
    ...
    console.log(JSON.stringify({ __pi_code_grab__: true, data: data }));
  }
  ...
  function onKeyDown(e) {
    if (e.key === 'Escape') {
      e.preventDefault();
      e.stopPropagation();
      console.log(JSON.stringify({ __pi_code_grab__: true, cancelled: true }));
    }
  }

  /* ── Cursor style override ── */
  var cursorStyle = document.createElement('style');
  cursorStyle.id = '__pi_code_inspector_cursor__';
  ...
  window.__pi_code_inspector_cleanup__ = function() {
    document.removeEventListener('mousemove', onMouseMove, true);
    document.removeEventListener('click', onClick, true);
    document.removeEventListener('keydown', onKeyDown, true);
    if (overlay.parentNode) overlay.parentNode.removeChild(overlay);
    if (cursorStyle.parentNode) cursorStyle.parentNode.removeChild(cursorStyle);
    delete window.__pi_code_inspector_active__;
    delete window.__pi_code_inspector_cleanup__;
  };
})();`;
}
...
export function getCleanupScript(): string {
  return `if (window.__pi_code_inspector_cleanup__) window.__pi_code_inspector_cleanup__();`;
}
