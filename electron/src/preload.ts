...
try {
  ...
  ipcRenderer.invoke("app:getGlassSupported").then((supported: boolean) => {
    ...
    const transparencySetting = globals.localStorage?.getItem("pi-code-transparency") ?? null;
  ...
  // back to "dark" when pi-code-theme is unset — avoids a system→dark flash.
  const storedTheme = globals.localStorage?.getItem("pi-code-theme");
...
contextBridge.exposeInMainWorld("claude", {
  ...
  git: {
    ...
    diffFile: (cwd: string, file: string, staged: boolean, group?: "staged" | "unstaged" | "untracked") =>
      ipcRenderer.invoke("git:diff-file", { cwd, file, staged, group }),
