      ...
      if (!squirrelReady) {
        ...
        try {
          ...
          const releaseUrl = lastDownloadedVersion
            ? `https://github.com/notshekhar/pi-code/releases/tag/v${lastDownloadedVersion}`
...
export function findUpdateZip(): string | null {
  // electron-updater stores downloads in ~/Library/Caches/pi-code-updater/pending/
  // app.getPath("appData") = ~/Library/Application Support, so go up one to ~/Library/
  const cacheDir = path.join(path.dirname(app.getPath("appData")), "Caches", "pi-code-updater", "pending");
  if (!fs.existsSync(cacheDir)) return null;

  // Try exact match first (e.g. Pi Code-0.6.1-arm64-mac.zip)
  if (lastDownloadedVersion) {
...
async function manualMacInstall(): Promise<void> {
  ...
  // e.g. /Applications/Pi Code.app/Contents/MacOS/Pi Code → /Applications/Pi Code.app
  ...
  const tmpDir = path.join(app.getPath("temp"), `pi-code-update-${Date.now()}`);
