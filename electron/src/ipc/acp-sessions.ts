...
type AcpResolvedConfigOption = {
  id: string;
  name: string;
  category?: "model" | "mode" | string | null;
  type: "select";
  currentValue: string;
  options: Array<{
    value: string;
...
function hasSynthesizedCategory(
  configOptions: AcpResolvedConfigOption[],
  category: "model" | "mode",
): boolean {
  return configOptions.some((option) => option.category === category || option.id === category);
}

function sortAcpConfigOptionsForDisplay(options: AcpResolvedConfigOption[]): AcpResolvedConfigOption[] {
  const rank = (o: AcpResolvedConfigOption): number => {
    if (o.category === "mode" || o.id === "mode") return 0;
    if (o.category === "model" || o.id === "model") return 1;
    return 2;
  };
...
/** Agents often send a partial config_option_update; keep mode/model rows we synthesized or buffered earlier. */
function mergeConfigOptionUpdatePreservingSynthesized(
  incoming: AcpResolvedConfigOption[],
  previous: AcpResolvedConfigOption[] | undefined,
): AcpResolvedConfigOption[] {
  const merged = [...incoming];
  if (previous?.length) {
    for (const cat of ["mode", "model"] as const) {
      if (!hasSynthesizedCategory(merged, cat)) {
        const preserved = previous.find((o) => o.category === cat || o.id === cat);
        if (preserved) merged.push(preserved);
      }
    }
  }
  return sortAcpConfigOptionsForDisplay(merged);
}

/** Merge configOptions from session response, event buffer, and ACP mode/model state. */
function resolveConfigOptions(
  sessionResult: { configOptions?: unknown[] | null; models?: unknown | null; modes?: unknown | null },
  internalId: string,
  logLabel: string,
): unknown[] {
  const fromResponse = (sessionResult.configOptions ?? []) as unknown[];
  const fromEvents = (configBuffer.get(internalId) ?? []) as unknown[];
  const configOptions = [...((fromResponse.length ? fromResponse : fromEvents) as AcpResolvedConfigOption[])];

  const modes = (sessionResult as Record<string, unknown>).modes as AcpModeState;
  if (!hasSynthesizedCategory(configOptions, "mode") && modes?.availableModes?.length) {
    log(logLabel, `Synthesizing mode config option from ${modes.availableModes.length} ACP modes`);
    configOptions.push({
      id: "mode",
      name: "Mode",
      category: "mode",
  ...
  // Fallback: synthesize model config option from unstable models API when the
  // agent did not provide one through stable configOptions.
  const models = (sessionResult as Record<string, unknown>).models as AcpModelState;
  if (!hasSynthesizedCategory(configOptions, "model") && models?.availableModels?.length) {
    log(logLabel, `Synthesizing model config option from ${models.availableModels.length} unstable ACP models`);
    configOptions.push({
      id: "model",
      name: "Model",
      category: "model",
  ...
  const sorted = sortAcpConfigOptionsForDisplay(configOptions);
  if (sorted.length) configBuffer.set(internalId, sorted);
  log(
    logLabel,
    `${sorted.length} config options (response=${fromResponse.length}, buffered=${fromEvents.length}, modes=${modes?.availableModes?.length ?? 0}, models=${models?.availableModels?.length ?? 0})`,
  );
  return sorted;
}
  ...
  const connection = new acp.ClientSideConnection((_agent) => ({
    async sessionUpdate(params: Record<string, unknown>) {
      ...
      if (eventKind === "config_option_update") {
        const incoming = ((update as { configOptions?: AcpResolvedConfigOption[] }).configOptions ?? []) as AcpResolvedConfigOption[];
        const prev = configBuffer.get(internalId) as AcpResolvedConfigOption[] | undefined;
        const merged = mergeConfigOptionUpdatePreservingSynthesized(incoming, prev);
        configBuffer.set(internalId, merged);
        (update as { configOptions: AcpResolvedConfigOption[] }).configOptions = merged;
      }
...
export function register(getMainWindow: () => BrowserWindow | null): void {
  ...
  ipcMain.handle("acp:set-config", async (_event, { sessionId, configId, value }: { sessionId: string; configId: string; value: string }) => {
    ...
    try {
      ...
      const buffered = configBuffer.get(sessionId) as AcpResolvedConfigOption[] | undefined;
      ...
      if (configId === "mode") {
        ...
        log("ACP_CONFIG", `session=${sessionId.slice(0, 8)} mode=${value} OK (via setSessionMode)`);

        if (buffered) {
          const nextOptions = buffered.map((option) => (
            option.id === "mode" || option.category === "mode"
              ? { ...option, currentValue: value }
              : option
          ));
          configBuffer.set(sessionId, nextOptions);
          return { configOptions: nextOptions };
        }
      ...
      try {
        ...
        if (result.configOptions) {
          const merged = mergeConfigOptionUpdatePreservingSynthesized(
            result.configOptions as AcpResolvedConfigOption[],
            buffered,
          );
          configBuffer.set(sessionId, merged);
          return { configOptions: merged };
        }
