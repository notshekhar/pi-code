...
1. **`useSettings` hook** (renderer, localStorage) — UI preferences that only the renderer needs: model, permissionMode, panel widths, active tools, thinking toggle. Per-project settings keyed by `pi-code-{projectId}-*`, global settings keyed by `pi-code-*`.
...
- End with `---` separator and `**Full Changelog**: https://github.com/notshekhar/pi-code/compare/v{prev}...v{current}`
...
- **`shared/types/codex.ts`** — re-exports with `Codex`-prefixed aliases (e.g., `CodexThreadItem`, `CodexSessionEvent`) plus Pi Code–specific wrappers (`CodexApprovalRequest`, `CodexRequestUserInputRequest`).
